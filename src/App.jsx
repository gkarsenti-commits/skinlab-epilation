...
const DOCTOLIB_URL = "https://www.doctolib.fr/centre-laser-et-esthetique/montpellier/skinlab";
const WHATSAPP_URL = "https://api.whatsapp.com/send/?phone=33422916022&text&type=phone_number&app_absent=0";
const INSTAGRAM_URL = "https://www.instagram.com/skinlab.cliniqueopera";
const MAPS_URL = "https://www.google.com/maps/search/?api=1&query=2%20rue%20des%20%C3%A9tuves%2034000%20Montpellier";
...
const faq = [
  {
    ...
    a: "On parle plutôt de réduction durable de la pilosité. Le résultat dépend du phototype, de la couleur du poil, de la zone, des facteurs hormonaux et de la régularité des séances.",
  },
  {
    q: "Combien de séances faut-il prévoir ?",
    a: "Plusieurs séances sont nécessaires, car tous les poils ne sont pas dans la même phase de croissance au même moment. Le protocole est personnalisé après évaluation.",
  },
  {
    q: "Peut-on traiter les peaux mates ?",
    a: "Le Clarity II permet d’adapter les paramètres au phototype. L’indication et les réglages sont confirmés avant le traitement afin de privilégier la sécurité cutanée.",
  },
  {
    q: "Faut-il éviter le soleil ?",
    a: "Oui. Le bronzage récent augmente le risque d’effets indésirables. Les consignes exactes sont précisées avant la séance selon la zone et le phototype.",
  },
  {
    q: "Les poils blancs ou très clairs répondent-ils ?",
    a: "Les poils blancs, roux ou très peu pigmentés répondent mal au laser, car le ciblage dépend en grande partie de la mélanine du poil.",
  },
];
    ...
    <div className="min-h-screen bg-[#f7f3ed] text-stone-950">
      <header className="sticky top-0 z-50 border-b border-stone-200/70 bg-[#f7f3ed]/85 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 md:px-8">
          <div>
            ...
            <div className="text-xs uppercase tracking-[0.22em] text-stone-500">by Clinique Opéra · Montpellier</div>
          </div>
          ...
          <Button href={DOCTOLIB_URL} className="rounded-full bg-stone-950 px-5 text-white hover:bg-stone-800">Prendre RDV</Button>
        </div>
      </header>
      ...
      <main>
          ...
          <div className="relative mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-[1.05fr_0.95fr]">
            <div>
              <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-stone-300 bg-white/60 px-4 py-2 text-sm text-stone-700 shadow-sm">
                <Icon name="sparkles" className="h-4 w-4" /> Épilation laser à Montpellier
              </div>
              ...
              <p className="mt-7 max-w-2xl text-lg leading-8 text-stone-600 md:text-xl">
                Chez SkinLab by Clinique Opéra, l’épilation laser est réalisée avec Clarity II, une technologie qui permet d’adapter le traitement à votre peau, votre pilosité et votre objectif.
              </p>

              <div className="mt-9 flex flex-col gap-3 sm:flex-row">
                <Button href={DOCTOLIB_URL} className="h-12 rounded-full bg-stone-950 px-7 text-white hover:bg-stone-800">
                  Prendre rendez-vous <Icon name="arrow" className="h-4 w-4" />
                </Button>

                <Button href={WHATSAPP_URL} variant="outline" className="h-12 rounded-full border-stone-300 bg-white/60 px-7 hover:bg-white">
                  Écrire sur WhatsApp
                </Button>
              </div>
              ...
              <div className="rounded-[2rem] border border-white/70 bg-white/55 p-4 shadow-2xl shadow-stone-300/40 backdrop-blur">
                <div className="relative min-h-[520px] overflow-hidden rounded-[1.6rem] bg-stone-950 p-8 text-white">
                  ...
                  <div className="relative z-10 flex h-full flex-col justify-between">
                    <div>
                      <div className="mb-5 inline-flex rounded-full bg-white/10 px-4 py-2 text-sm">Clarity II</div>
                    ...
                    <div className="mt-40 grid gap-4 sm:grid-cols-2">
                      <div className="rounded-2xl bg-white/10 p-5 backdrop-blur">
                        ...
                        <p className="mt-1 text-sm text-white/65">Technologie pensée pour traiter les zones de façon fluide.</p>
                      </div>

                      <div className="rounded-2xl bg-white/10 p-5 backdrop-blur">
                        <Icon name="shield" className="mb-3 h-5 w-5" />
                        <div className="font-medium">Confort cutané</div>
                        <p className="mt-1 text-sm text-white/65">Paramètres adaptés selon la zone, la peau et la pilosité.</p>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <section id="clarity" className="px-5 py-20 md:px-8">
          <div className="mx-auto max-w-7xl">
            <SectionTitle
              ...
              text="Un laser performant ne remplace jamais l’indication personnalisée : il permet surtout d’adapter le traitement avec précision à chaque peau et à chaque pilosité."
            />

            <div className="mt-12 grid gap-5 md:grid-cols-3">
              {[
                ["zap", "Technologie adaptable", "Clarity II permet d’adapter les paramètres selon le phototype, la zone et la densité pilaire."],
                ["timer", "Traitements fluides", "Une plateforme pensée pour traiter efficacement les petites comme les grandes zones."],
                ["shield", "Confort & sécurité", "Le protocole est ajusté selon la peau, l’exposition solaire et les contre-indications éventuelles."],
              ].map(([icon, title, text]) => (
        ...
        <section id="faq" className="bg-white px-5 py-20 md:px-8">
          <div className="mx-auto max-w-4xl">
            ...
            <div className="mt-10">{faq.map((item, index) => <FAQItem key={item.q} item={item} index={index} />)}</div>
          </div>
        </section>
          ...
          <div className="mx-auto max-w-7xl overflow-hidden rounded-[2rem] bg-stone-950 text-white shadow-2xl shadow-stone-300/50">
            <div className="grid gap-8 p-8 md:p-12 lg:grid-cols-[1fr_0.8fr] lg:p-16">
              <div>
                ...
                <p className="mt-5 max-w-2xl text-lg leading-8 text-white/70">Prenez rendez-vous en ligne pour une séance d’épilation laser chez SkinLab by Clinique Opéra, ou contactez l’équipe directement sur WhatsApp.</p>

                <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                  <Button href={DOCTOLIB_URL} variant="light"><Icon name="calendar" className="h-4 w-4" /> Prendre RDV sur Doctolib</Button>
                  <Button href={WHATSAPP_URL} variant="outlineDark"><Icon name="phone" className="h-4 w-4" /> WhatsApp</Button>
                </div>
              </div>

              <div className="rounded-[1.5rem] bg-white/10 p-6">
                <div className="flex gap-3"><Icon name="pin" className="mt-1 h-5 w-5" /><div><div className="font-medium">SkinLab by Clinique Opéra</div><p className="mt-1 text-sm leading-6 text-white/65">2 rue des Étuves, 34000 Montpellier. À proximité de l’Opéra Comédie.</p></div></div>

                <div className="mt-6 overflow-hidden rounded-[2rem] border border-white/10 bg-white/5 p-2">
                  <img src={mapImage} alt="Plan d’accès SkinLab by Clinique Opéra Montpellier" className="w-full rounded-[1.5rem] object-cover" />
                </div>

                <div className="mt-6 grid gap-3 border-t border-white/10 pt-6 text-sm leading-7 text-white/65">
                  <a href={MAPS_URL} target="_blank" rel="noreferrer" className="hover:text-white">Voir l’itinéraire Google Maps</a>
                  <a href={WHATSAPP_URL} target="_blank" rel="noreferrer" className="hover:text-white">Nous contacter sur WhatsApp</a>
                  <a href={INSTAGRAM_URL} target="_blank" rel="noreferrer" className="hover:text-white">Instagram @skinlab.cliniqueopera</a>
                </div>
              </div>
            </div>
          </div>
      ...
      <footer className="border-t border-stone-200 px-5 py-8 text-sm text-stone-500 md:px-8">
        <div className="mx-auto flex max-w-7xl flex-col justify-between gap-3 md:flex-row">
          <div>© {year} SkinLab by Clinique Opéra · Épilation laser Clarity II</div>
          <div className="flex gap-5"><a href={DOCTOLIB_URL} target="_blank" rel="noreferrer">Doctolib</a><a href={WHATSAPP_URL} target="_blank" rel="noreferrer">WhatsApp</a><a href={INSTAGRAM_URL} target="_blank" rel="noreferrer">Instagram</a></div>
        </div>
      </footer>
    </div>
  );
